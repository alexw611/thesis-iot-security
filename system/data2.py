...
def run_measurement():
    ...
    csv_filename = f"{OUTPUT_DIR}/messdaten_secure_{timestamp}.csv"
    
    print("=" * 70)
    print("  MESSDATEN-ERFASSUNG - GEHÄRTETES SYSTEM")
    print("  AES-128-CTR + HMAC-SHA256")
    print("=" * 70)
    ...
    camera_names = [cam["name"] for cam in initial_data["cameras"]]
    ...
    for cam in camera_names:
        if all_measurements[cam]:
            ...
            min_fps = min(m["fps_min"] for m in measurements if m["fps_min"] > 0)
            max_fps = max(m["fps_max"] for m in measurements)
            
            print(f"  {cam}:")
            print(f"    Ø FPS:        {avg_fps:.1f}")
            print(f"    FPS Bereich:  {min_fps:.1f} - {max_fps:.1f}")
    ...
    print("=" * 70)
    print("✅ Fertig! Dateien bereit für Excel-Import")
    print("=" * 70)
